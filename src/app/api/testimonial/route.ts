...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (error) {
      ...
      if (error.code === '23505') { 
          return NextResponse.json({ error: 'Email already used for this business' }, { status: 409 });
      }
      return NextResponse.json({ error: 'A database error occurred during the submission process.' }, { status: 500 });
    }

    const responseText = data as string;

    if (responseText.includes('successfully')) {
      return NextResponse.json({ message: 'Thank you for your testimonial!', status: 'success' });
    } else {
